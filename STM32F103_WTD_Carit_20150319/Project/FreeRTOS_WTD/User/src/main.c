...
#define mainCMD_TASK_STACK_SIZE             ( configMINIMAL_STACK_SIZE + 128 )
 ...
 * via a queue.
 */
static void vGetdataTask(void *pvParameters);
static void vModbusTask( void *pvParameters );
static void vPollTask( void *pvParameters );
static void vCmd_Task(void *pvParameters);
...
/*-----------------------------------------------------------*/
extern u8 Rec_Over_Flag, USART_RX_BUF[64];
int main( void )
{
#ifdef DEBUG
    debug();
    ...
    xTaskCreate( vGetdataTask, "MBGetDataTask", mainGETDATA_TASK_STACK_SIZE, NULL, mainGETDATA_TASK_PRIORITY, NULL );
    xTaskCreate( vCmd_Task, "cmdTask", mainCMD_TASK_STACK_SIZE, NULL, mainCMD_TASK_PRIORITY, NULL );
...
static void prvSetupHardware( void )
{
    ...
    *( ( unsigned long * ) 0x40022000 ) = 0x02;
    ...
    Driver_UsartInit(USART1, 9600, USART_WordLength_8b, USART_StopBits_1, USART_Parity_No);
    //Driver_UsartInit(USART2, 9600, USART_WordLength_8b, USART_StopBits_1, USART_Parity_No);
    Driver_UsartInit(USART3, 9600, USART_WordLength_8b, USART_StopBits_1, USART_Parity_No);
...
/*-----------------------------------------------------------*/
void vGetdataTask(void *pvParameters)
{
    u16 pm2x,pm10x;
    char noise[5];
    float sound;
    ...
    while(1)
    {
        ...
        if(Rec_Over_Flag==1)
        {
            ...
            {
                pm2x = *(USART_RX_BUF+5)<<8 | *(USART_RX_BUF+6);
                pm10x = *(USART_RX_BUF+9)<<8 | *(USART_RX_BUF+10);
                printf("%d\n", pm2x/10);
                printf("%d\n", pm10x/10);
                //Driver_UsartSendData(USART1, USART_RX_BUF, 18);
            }
            else if(*USART_RX_BUF=='A')
            {
                noise[0] = *(USART_RX_BUF+6);
                noise[1] = *(USART_RX_BUF+7);
                noise[2] = *(USART_RX_BUF+8);
                noise[3] = *(USART_RX_BUF+9);
                noise[4] = '\0';
                
                sound = atof(noise);
                printf("%lf",sound);
//                 printf("%c",*(USART_RX_BUF+6));
//                 printf("%c",*(USART_RX_BUF+7));
//                 printf("%c\n",*(USART_RX_BUF+9));
                //Driver_UsartSendData(USART1, USART_RX_BUF, 18);
            }
        }
...
{
    u8 cmd[] = {'A','W','A','A'};
    //u8 cmd[]={0x01,0x04,0x00,0x00,0x00,0x04,0xf1,0xc9};

    while(1)
    {
        Driver_UsartSendData(USART3, cmd, sizeof(cmd));
        vTaskDelay(4000);
    }
}
/*-----------------------------------------------------------*/
